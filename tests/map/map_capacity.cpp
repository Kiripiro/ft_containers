...
static void my_map()
{
	TESTED_NAMESPACE::map<std::string, char> map;
	TESTED_NAMESPACE::map<std::string, char> second;

	map["hello"] = 'h';
	map["world"] = 'w';
	map["!"] = '!';
	std::cout << "map:";
	for (TESTED_NAMESPACE::map<std::string, char>::iterator it = map.begin(); it != map.end(); ++it)
		std::cout << " " << it->first;
	std::cout << std::endl;
